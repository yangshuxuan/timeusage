...
object TimeUsage extends TimeUsageInterface {
  ...
  def row(line: List[String]): Row = {
    val xh::xs = line
    val result = xh :: xs.map(_.toDouble)
    Row.fromSeq(result)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def getColumns(cond:List[String],notCond:List[String]):List[Column]={
      columnNames.withFilter(v => cond.exists(v.startsWith(_)) && ! notCond.exists(v.startsWith(_)) ).map(col(_))
    }
    (getColumns(primaryNeedsStart,Nil),getColumns(workingActivitiesStart,Nil),getColumns(otherActivities,primaryNeedsStart:::workingActivitiesStart))
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    //val p = summed.groupByKey(v =>(v.working,v.sex,v.age)).agg(typed.avg(_.primaryNeeds),typed.avg(_.work),typed.avg(_.other))
    val p = summed.groupByKey(v =>(v.working,v.sex,v.age)).agg(
      round(avg($"primaryNeeds"),1).as[Double],
      round(avg($"work"),1).as[Double],
      round(avg($"other"),1).as[Double])

    p.map({case((working,sex,age),primaryNeeds,work,other) => TimeUsageRow(working,sex,age,primaryNeeds,work,other)}).sort("working","sex","age")
  }
}
